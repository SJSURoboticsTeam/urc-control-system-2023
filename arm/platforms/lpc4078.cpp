...
hal::result<application_framework> initialize_platform() 
{
  ...
  HAL_CHECK(hal::rmd::mc_x::create(can_router, counter, 8.0, 0x141));
  static auto rotunda_mc_x_servo = 
  HAL_CHECK(hal::make_servo(rotunda_mc_x, 100.0_rpm));

  static auto shoulder_mc_x =
  HAL_CHECK(hal::rmd::mc_x::create(can_router, counter, 8.0, 0x141));
  static auto shoulder_mc_x_servo = HAL_CHECK(hal::make_servo(shoulder_mc_x, 100.0_rpm));

  static auto elbow_mc_x = 
  HAL_CHECK(hal::rmd::mc_x::create(can_router, counter, 8.0, 0x141));
  static auto elbow_mc_x_servo = HAL_CHECK(hal::make_servo(elbow_mc_x, 100.0_rpm));

  static auto left_wrist_mc_x =
  HAL_CHECK(hal::rmd::mc_x::create(can_router, counter, 8.0, 0x141));
  static auto left_wrist_mc_x_servo = HAL_CHECK(hal::make_servo(left_wrist_mc_x, 100.0_rpm));

  static auto right_wrist_mc_x =
  HAL_CHECK(hal::rmd::mc_x::create(can_router, counter, 8.0, 0x141));
  static auto right_wrist_mc_x_servo = HAL_CHECK(hal::make_servo(right_wrist_mc_x, 100.0_rpm));
  ...
  return application_framework{.rotunda_servo = &rotunda_mc_x_servo,
