...
int main()
{
  ...
  // storage to memory.
  if (!sjsu::arm_mimic::initialize_processor()) {
    hal::halt();
  }

  auto application_resource = sjsu::arm_mimic::initialize_platform();

  if (!application_resource) {
    hal::halt();
  }

  auto is_finished = sjsu::arm_mimic::application(application_resource.value());
