...
namespace Drive
{
    ...
    public:
        ...
        drive_commands SwitchSteerMode(drive_commands commands, tri_wheel_router_arguments previous_arguments, motor_feedback current_motor_feedback, hal::serial& terminal)
        {
            ...
            bool steers_stopped((current_motor_feedback.left_steer_speed >= -0.01f && current_motor_feedback.left_steer_speed <= 0.01f) && (current_motor_feedback.right_steer_speed >= -0.01f && current_motor_feedback.right_steer_speed <= 0.01f) && (current_motor_feedback.back_steer_speed >= -0.01f && current_motor_feedback.back_steer_speed <= 0.01f));
    ...
    private:
        char previous_mode_ = 'H'; // This is H for homing when the rover turns on, it makes sure that we don't switch to drive mode and allow commands to be parsed through
