...
inline tri_wheel_router_arguments drive_steering(drive_commands commands)
{
  ...
  if (commands.angle > 0) {
    ...
    steer_arguments.right.steer.angle = calculate_ackermann(outter_wheel_angle);
  } else if (commands.angle < 0) {
    outter_wheel_angle = static_cast<float>(-commands.angle);
    steer_arguments.right.steer.angle = outter_wheel_angle;
    steer_arguments.left.steer.angle = calculate_ackermann(outter_wheel_angle);
  }
  if (commands.angle == 0) {
  ...
  if (outter_wheel_angle > 0) {
    steer_arguments.right.hub.speed = -get_inner_wheel_hub_speed(
      static_cast<float>(commands.speed), outter_wheel_angle);
    steer_arguments.left.hub.speed = -static_cast<float>(commands.speed);
    steer_arguments.back.hub.speed = get_back_wheel_hub_speed(
      static_cast<float>(commands.speed), outter_wheel_angle);
  }

  else if (outter_wheel_angle < 0) {
    steer_arguments.left.hub.speed = -get_inner_wheel_hub_speed(
      static_cast<float>(commands.speed), outter_wheel_angle);
  ...
  steer_arguments.left.steer.angle += left_leg_drive_offset;
  steer_arguments.right.steer.angle += right_leg_drive_offset;
  steer_arguments.back.steer.angle += back_leg_drive_offset;
  return steer_arguments;
}

inline tri_wheel_router_arguments spin_steering(drive_commands commands)
{
  tri_wheel_router_arguments temp;
  temp.back.steer.angle = back_leg_spin_offset;
  temp.left.steer.angle = left_leg_spin_offset;
  temp.right.steer.angle = right_leg_spin_offset;
...
inline tri_wheel_router_arguments translate_steering(drive_commands commands)
{
  tri_wheel_router_arguments steer_arguments;

  steer_arguments.left.steer.angle =
    (static_cast<float>(-commands.angle) + left_leg_drive_offset);
  steer_arguments.right.steer.angle =
    (static_cast<float>(-commands.angle) + right_leg_drive_offset);
  steer_arguments.back.steer.angle =
...
inline float calculate_ackermann(float outter_wheel_angle)
{
