...
public:
  ...
  void start()
  {
    std::array<hal::byte, 2> start_address{ { start_first_half,
                                              start_second_half } };
    hal::write(m_i2c, addresses::address, start_address, hal::never_timeout());
  }
  ...
  int16_t read_co2()
  {
    ...
    // hal::never_timeout());
    hal::write(m_i2c, addresses::address, read_address);
  ...
  void stop()
  {
    std::array<hal::byte, 2> stop_address{ { stop_first_half,
                                             stop_second_half } };

    hal::write(m_i2c,
               addresses::address,
               std::span{ stop_address },
               hal::never_timeout());
  }
